...
void merge_RMatrixTH2D_bjet()
{
    const int first = 0;
    const int last  = 8; // 9 for QCD, 8 for bjet

    const char *pattern =
        "/data_CMS/cms/mnguyen/bJetAggRun3/PPRef2024/bJet/"
    ...
    TFile *fout = TFile::Open(
        ...
        "RMatrix_Run3_btagWP712_template_for_fit_histos_3D_bjet_f_80_9999_2_merged.root",
        // "TestAdditionofBlocks.root", 
        "RECREATE"
    );
   ...
   // Test output # entries
    ...
    fout->cd();
    // fout->Write(); // Dont re write the objects! 
    fout->Close();
