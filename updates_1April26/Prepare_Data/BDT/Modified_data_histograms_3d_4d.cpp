...
void do_hist(TString &filename,  TString &dataset, Float_t &pT_low, Float_t &pT_high, bool &aggregated, double &BDTcutTh, bool &btag, Int_t &n, Int_t &lowEn, Int_t &highEn, bool &data, TString folder){
    ...
    */
    ...
    TH3D *h3D_test = new TH3D("h3D_test", "h3D_test", 200, 0, 20,
                                                     100, 0, 1
                                                    3, 80, 140);
    h3D_test->Sumw2();

    TH3D *h3D_testIn = new TH3D("h3D_testIn", "h3D_testIn", bins_mb, mb_binsVector,
                                       bins_dr, dr_binsVector,
                                       jtpt_bins, jtpt_binsVector);
    h3D_testIn->Sumw2();
    ...
    for (Long64_t ient = 0; ient < n_entries; ient++) { //
        ...
        for (Int_t ijet = 0; ijet < t.nref; ijet++) {
            ...
            if (aggregated){
                ...
                if (reco_hadrons_4vec.size() !=  2) continue; // skip jet 
                ...
                h3D_testIn->Fill(mB, dr, jtpt, eec);
              ...
              // --- Compare filing th3d here ? 


              // While filling here is ok! 
              // test fill TH3D first! 
              h3D_test->Fill(mB, dr, jtpt, eec); 
...
// -- for test 
TH1D* h3dproj_mass = (TH1D*) h3D_test->ProjectionX("h3dproj_mass", 1, h3D_test->GetNbinsY(), 1, h3D_test->GetNbinsZ());
h3dproj_mass->Write();
