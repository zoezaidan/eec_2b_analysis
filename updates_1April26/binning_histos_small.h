...
// Equally sized bins in logarithmic scale.
...
/*
// Cannot execute function in the header
void build_dr_bins()
{   
 ...
 const Double_t end   = 1;
    ...
    Double_t step = (log_end - log_start) / (dr_binsVectorSize - 1);

    for (Int_t i = 0; i < dr_binsVectorSize; ++i)
    {
        dr_binsVector[i] = std::exp(log_start + step * i);
        cout <<  dr_binsVector[i] << ", " << endl;
    }
}
...
// -- Another mass binning 
const int mblarge_binsVectorSize = 21;
int mblarge_bins = mblarge_binsVectorSize -1;
Double_t mblarge_binsVector[mblarge_binsVectorSize] = {
    0., 
    1.,
    2.,
    3.,
