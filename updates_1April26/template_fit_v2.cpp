...
TString sDirname = "TemplateFitOutputPlots31mars_HLT80_HGdataonly";
...
void do_template_fit_wdata(TString &dataset, TString &templates, TString &templates_bjet, TString pT_selection, TString folder, TString &fout_name, bool& alsoLowEG){
    /*
    This fit use sig and bkg templates from the input files.
    */
    // Dataset: for now HGtrigered data. Lowe EG triggered daa to be added next.
    //-- Samples: Dijet + bjet 
    ...
    TFile *file_data = new TFile(dataset, "read");
        ...
        if(!h3D_MC_pseudo){Error("Get:", "histogram does not exist '%s' ",h3D_MC_pseudo->GetName()); return;}
        if(!h3D_MC_pseudo_bjet){Error("Get:", "histogram does not exist '%s' ",h3D_MC_pseudo_bjet->GetName()); return;}

    //Get number of bin entries
    Int_t bins_pt = h3D_data->GetNbinsZ();
    Int_t bins_dr = h3D_data->GetNbinsY(); // Re-defined bins(exists in the header file)
...
// return;
    ...
    // for(Int_t ibin_pt = 1; ibin_pt <= 1; ibin_pt++){
        ...
        // Bin = 0: integaretd deltaR range, other bins are the DeltaR bins 
            ...
            double integral_inputdata = h_data_mb->Integral(2, mb_bins-1,"width");
...
void template_fit_v2(){

    //Get data and mc labels
    TString pT_selection = "80_140";// full range 
    TString folder = "/home/llr/cms/shatat/CMSAnalysis/EECs/";
    ...
    // Without sampling: trivail MC as asum is in this file 
    // TString dataset_nosample =  "template_for_fit_histos_3D_withoutsampling_makevtx_btag.root";
    // TString templates_nosample = "template_for_fit_histos_3D_withoutsampling_makevtx_btag.root";
    // TString fout_name_nosample = folder + "histos_3d_from_templ_data_nosample_makevtx_" + pT_selection + ".root";
    // TString fout_name_nosample_trivialtest = folder + "histos_3d_from_templ_data_nosample_makevtx_" + pT_selection + "_trivialMC" + ".root"; // only for trivial test

    // After splitting the samples via Random generator: bjet sample 
    // TString dataset   = "template_for_fit_histos_3D_makevtx_btag.root";
    // TString templates = "template_for_fit_histos_3D_makevtx_btag.root";
    // TString fout_name = folder + "histos_3d_from_templ_data_makevtx_" + pT_selection + ".root";
    // TString fout_name_trivialtest = folder + "histos_3d_from_templ_data_makevtx_" + pT_selection  + "_trivialMC" +  ".root";

    // -- Using Dijet sample:
    // TString dataset_dijet = "Dijet_template_for_fit_histos_3D_makevtx_btag.root"; // 
    // TString templates_dijet =  "Dijet_template_for_fit_histos_3D_makevtx_btag.root";
    // TString fout_name_dijet = folder + "histos_3d_from_templ_data_makevtx_BDTcut" + pT_selection + "_dijet" +  ".root";


/*
// -- Trivial tests 
    // -- without sample seperation 
    do_template_fit_trivialtest(dataset_nosample, templates_nosample, pT_selection, folder, fout_name_nosample_trivialtest);
   
    // - -with samples seperated 
    do_template_fit_trivialtest(dataset, templates, pT_selection, folder, fout_name_trivialtest);
    
    std::vector<std::unique_ptr<TCanvas>> vcanvas_nosample_trivialtest(jtpt_bins);
    std::vector<std::unique_ptr<TCanvas>> vcanvas_trivialtest (jtpt_bins);

    for (int pt_bin = 1; pt_bin <= jtpt_bins;pt_bin++)
    {
        vcanvas_trivialtest [pt_bin - 1] = 
            draw_template_fit_result(fout_name_trivialtest, dataset, folder, pT_selection, pt_bin);
        vcanvas_nosample_trivialtest [pt_bin - 1] = 
            draw_template_fit_result(fout_name_nosample_trivialtest, dataset_nosample, folder, pT_selection, pt_bin);
    }

    TFile *file = new TFile("Summary_histo_templatefit.root", "RECREATE");
    for (const auto& c : vcanvas_trivialtest) {if(c) c->Write();}  
    for (const auto& c : vcanvas_nosample_trivialtest) {if(c) c->Write();}  
...
/*
// --- MC Closure tests: bjet sample 
// -- Test template fit using independent samples: with full MC (not trivial sum of sig and MC): using bjet sample only
    // do_template_fit(dataset, templates, pT_selection, folder, fout_name);
    TString sfoutputPlots = "Summary_histo_templatefit.root";
    TFile *foutputPlots = new TFile(sfoutputPlots, "RECREATE"); // For canvas output
    if (!foutputPlots || foutputPlots->IsZombie()) {
        std::cout << "Error opening file!" << std::endl;
        return;
    }
    // Draw: for the integarted deltaR bin, one plot Vs. ptbins 
    draw_template_fit_result(fout_name, foutputPlots, dataset, folder, pT_selection, 0,  true);
    // Draw for DeltaR bins 
    for (int pt_bin = 1; pt_bin <= jtpt_bins;pt_bin++)
    {
        draw_template_fit_result(fout_name, foutputPlots, dataset, folder, pT_selection, pt_bin);
    }
...
*/
 ...
 // --- MC closure test using Dijet sample 
/*
    // -- test using Dijet sample and b jet sample to combine them and see how the hist changes  
    do_template_fit_Dijet(dataset_dijet, templates_dijet, pT_selection, folder, fout_name_dijet);
    draw_template_fit_result(fout_name_dijet, foutputPlots_dijet, dataset_dijet, folder, pT_selection, 0,  true);
    for (int pt_bin = 1; pt_bin <= jtpt_bins;pt_bin++)
    {
        draw_template_fit_result(fout_name_dijet, foutputPlots_dijet, dataset_dijet, folder, pT_selection, pt_bin);
    }
*/      

/*
    // -- Test the combination of Dijet and bjet samples 
        /////////   use input files from Zaidan work at /data_CMS/cms/zaidan/analysis_lise/
        //// using Franc bdt cut at -.90 
        // Bjet: template_for_fit_histos_3D_step_1_bjet_btag_BDTcut.root 
        // Dijet: template_for_fit_histos_3D_step_1_qcd_btag_BDTcut.root
        //// Or with Lidas cut at 0.3 
        // template_for_fit_histos_3D_step_1_bjet_btag_BDT_cut_test_LidasCut.root 
        // and template_for_fit_histos_3D_step_1_qcd_btag_BDTcut_test_LidasCut.root

        // -- test using Franc BDT cut 
            // -- Using modified templates by Zoe, 
        // TString file_tagname = "test_combine_dijet_bjet_nosample1";
        //-- using Franc wp  
        TString dataset_dijet_nosample1 = "template_for_fit_histos_3D_step_1_qcd_btag_BDTcut.root"; // 
        TString templates_dijet_nosample1 =  dataset_dijet_nosample1;
        TString dataset_bjet_nosample1 = "template_for_fit_histos_3D_step_1_bjet_btag_BDTcut.root"; // 
        TString templates_bjet_nosample1 = dataset_bjet_nosample1;
        TString fout_name_combinetest_nosample1 = "TEST_histos_3d_from_templ_data_BDTcut" + pT_selection + "_combinedDijetANDbjet" +  ".root";
        // do_template_fit_Dijet(dataset_dijet_nosample1, templates_dijet_nosample1, dataset_bjet_nosample1,  pT_selection, folder, fout_name_combinetest_nosample1);
        // draw_template_fit_result(fout_name_combinetest_nosample1, foutputPlots_dijet, dataset_dijet_nosample1, folder, pT_selection, 0,  true); // datasetname is not important
        // for (int pt_bin = 1; pt_bin <= jtpt_bins;pt_bin++)
        // {
        //     draw_template_fit_result(fout_name_combinetest_nosample1, foutputPlots_dijet, dataset_dijet_nosample1, folder, pT_selection, pt_bin);
        // }


        // -- test using  Lida BDT cut 
        TString dataset_dijet_nosample1_testBDTcut = "template_for_fit_histos_3D_step_1_qcd_btag_BDTcut_test_LidasCut.root"; 
        TString templates_dijet_nosample1_testBDTcut =  dataset_dijet_nosample1_testBDTcut;
        TString dataset_bjet_nosample1_testBDTcut = "template_for_fit_histos_3D_step_1_bjet_btag_BDT_cut_test_LidasCut.root";
        TString templates_bjet_nosample1_testBDTcut = dataset_bjet_nosample1_testBDTcut;
        TString fout_name_combinetest_nosample1_testBDTcut = "TEST_histos_3d_from_templ_data_testLidaBDTcut" + pT_selection + "_combinedDijetANDbjet" +  ".root";
        do_template_fit_Dijet(dataset_dijet_nosample1_testBDTcut, templates_dijet_nosample1_testBDTcut, dataset_bjet_nosample1_testBDTcut,  pT_selection, folder, fout_name_combinetest_nosample1_testBDTcut);
        draw_template_fit_result(fout_name_combinetest_nosample1_testBDTcut, foutputPlots_dijet, dataset_dijet_nosample1_testBDTcut, folder, pT_selection, 0,  true); // datasetname is not important
        for (int pt_bin = 1; pt_bin <= jtpt_bins;pt_bin++)
        {
            draw_template_fit_result(fout_name_combinetest_nosample1_testBDTcut, foutputPlots_dijet, dataset_dijet_nosample1_testBDTcut, folder, pT_selection,pt_bin);
        }
*/

/*
    // -- TEST: using Zoe templates (no sampling) with Lida. cut, 0B included in fit. Data is REAL 
        TString dataset =  "Prepare_Data/Data_Hist/withunder_overFlow_includedibins/hist_3d_4d_aggr_BDT_n1_data_HighEGJet_80_140.root"; // 
        TString templates_dijet_nosample = "template_for_fit_histos_3D_step_1_qcd_btag_BDTcut_test_LidasCut.root"; // Lida cut 
        TString templates_bjet_nosample = "template_for_fit_histos_3D_step_1_bjet_btag_BDT_cut_test_LidasCut.root";
        TString fout_name = "TEST_TemplateFits_histos_3d" + pT_selection +  ".root";
        // test the code works as a trivial test first befoer changing data hist  
        do_template_fit_wdata(dataset, templates_dijet_nosample, templates_bjet_nosample,  pT_selection, folder, fout_name);
*/
    /*
    // -- TEST: 30/03: updated MC templates, simialr cuts and under/overflow bin treatement
        TString dataset_HG = "Prepare_Data/Data_Hist/withunder_overFlow_includedibins/hist_3d_4d_aggr_BDT_n1_data_HighEGJet_80_140.root"; // 
        TString dataname = "allData";
        // These template versions were missing the HLT trigger condition but the prescale was applied 
            // MC_Templates_Result/BDT_365/template_for_fit_histos_3D_step_1_qcd_btag_BDT_cut.root 
            // MC_Templates_Result/BDT_365/template_for_fit_histos_3D_step_1_bjet_btag_BDT_cut.root
        // The updated version in directory: TEST_ALL_BDT_365 : HLT + prescale 
        TString templates_dijet = "MC_Templates_Result/TEST_ALL_BDT_365/template_for_fit_histos_3D_step_1_qcd_btag_BDT_cut.root";
        TString templates_bjet = "MC_Templates_Result/TEST_ALL_BDT_365/template_for_fit_histos_3D_step_1_bjet_btag_BDT_cut.root";
        
        TString fout_name = "TESTHLTprescale_TemplateFits_histos_3d_" + pT_selection +  ".root";

        do_template_fit_wdata(dataset_HG, templates_dijet, templates_bjet,  pT_selection, folder, fout_name);
        // draw_template_fit_result(fout_name, foutputPlots_dijet, dataname, folder, pT_selection, 0, true);
        // for (int pt_bin = 1; pt_bin <=jtpt_bins ;pt_bin++) // 
        // {
        //     draw_template_fit_result(fout_name, foutputPlots_dijet, dataname, folder, pT_selection, pt_bin);
        // }

    */

/*
    // -- 31/03/// Templates after updating template creating code for the trigger 
    TString dataset_HG = "Prepare_Data/Data_Hist/withunder_overFlow_includedibins/hist_3d_4d_aggr_BDT_n1_data_HighEGJet_80_140.root"; // and low EG is added inside code
        TString dataname = "allData";
        // The updated version in directory: TEST_ALL_BDT_365 : HLT + prescale 
        TString templates_dijet = "MC_Templates_Result/ALL_BDT_365/template_for_fit_histos_3D_step_1_qcd_HLT40_andprescale40_btag_BDT_cut.root";
        TString templates_bjet = "MC_Templates_Result/ALL_BDT_365/template_for_fit_histos_3D_step_1_bjet_HLT40_andprescale40_btag_BDT_cut.root";
        
        TString fout_name = "TESTHLTprescale40new_TemplateFits_histos_3d_" + pT_selection +  ".root";

        do_template_fit_wdata(dataset_HG, templates_dijet, templates_bjet,  pT_selection, folder, fout_name);

*/

    // -- 31/03/// Templates after updating template creating code for the trigger using HG data only
    alsoLowEG = false;
    TString dataset_HG = "Prepare_Data/Data_Hist/withunder_overFlow_includedibins/hist_3d_4d_aggr_BDT_n1_data_HighEGJet_80_140.root"; // and low EG is added inside code
        TString dataname = "allData";
        // The updated version in directory: TEST_ALL_BDT_365 : HLT80
        TString templates_dijet = "MC_Templates_Result/ALL_BDT_365/template_for_fit_histos_3D_step_1_qcd_HLT80_noprescale_btag_BDT_cut.root";
        TString templates_bjet = "MC_Templates_Result/ALL_BDT_365/template_for_fit_histos_3D_step_1_bjet_HLT80_noprescale_btag_BDT_cut.root";
        TString fout_name = "TESTHLTprescale80_HGdataonly_TemplateFits_histos_3d_" + pT_selection +  ".root";
        do_template_fit_wdata(dataset_HG, templates_dijet, templates_bjet,  pT_selection, folder, fout_name, alsoLowEG);
