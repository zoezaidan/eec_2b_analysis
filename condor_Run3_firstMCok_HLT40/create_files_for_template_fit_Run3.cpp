...
void make_templates(TString filename, TString output_folder, TString output_hist, TString domain,
                    ...
                    Long64_t ev_first = 0, Long64_t ev_last = -1, Int_t local_job_idx = -1) {
  ...
  double prescale_pf40 = 33.917210; // applied to data only (Run2)

  std::vector<TString> active_branches = {
    // reco branches — identical for data and MC
    "jtpt", "jteta", "nref", "jtNtrk", "jtNsvtx",

     // "discr_particleNet_BvsAll", for Run2 
     // New for Run3 
    ...
    "HLT_AK4PFJet40_v8"
    // "HLT_HIAK4PFJet60_v1", "HLT_HIAK4PFJet80_v1", "HLT_HIAK4PFJet100_v1",
     };
  if (isMC) {
  ...
  TH3D *h3D_0b = new TH3D("h3D_0b", "EEC weighted counts;m_{2B} [GeV];#DeltaR;Jet p_{T}", bins_mb, mb_binsVector, bins_dr, dr_binsVector, jtpt_bins, jtpt_binsVector);
  TH3D *h3D_b  = new TH3D("h3D_b",  "EEC weighted counts;m_{2B} [GeV];#DeltaR;Jet p_{T}", bins_mb, mb_binsVector, bins_dr, dr_binsVector, jtpt_bins, jtpt_binsVector);
  TH3D *h3D_bb = new TH3D("h3D_bb", "EEC weighted counts;m_{2B} [GeV];#DeltaR;Jet p_{T}", bins_mb, mb_binsVector, bins_dr, dr_binsVector, jtpt_bins, jtpt_binsVector);
  ...
  for (Long64_t ient = ev_first; ient < ev_last; ient++) {
    ...
    if (!isMC && dataType == 0) {
      ...
      // if (!(t.HLT_HIAK4PFJet40_v1)) continue; // Run 2 
      if (!(t.HLT_AK4PFJet40_v8)) continue; // Run 3
    }
    ...
    for (Int_t ijet = 0; ijet < t.nref; ijet++) {
      ...
      double btagVar = t. discr_unifiedParticleTransformer_probb[ijet] + t.discr_unifiedParticleTransformer_problepb[ijet] + t.discr_unifiedParticleTransformer_probbb [ijet];
      if (btag && btagVar <= 0.9) continue;
  ...
  TString job_suffix = (local_job_idx >= 0) ? Form("_job%d", local_job_idx) : "";
...
void create_files_for_template_fit_Run3(Int_t dataType = 1, Float_t pT_low = 80, Float_t pT_high = 200, Int_t n = 1, bool btag = true, bool isMC = true){
...
output_hist = Form("Run3_WP90_template_for_fit_histos_3D_qcd_file_0");
