...
void make_templates(const AnalysisConfig& cfg, Long64_t ev_first = 0, Long64_t ev_last = -1, Int_t job_idx = -1) {
  ...
  for (Long64_t ient = ev_first; ient < ev_last; ient++) {
    ...
    for (Int_t ijet = 0; ijet < t.nref; ijet++) {
      ...
      if (cfg.dataset.RunN == 3 && !cfg.dataset.isMC && t.HLT_HIAK4PFJet60_v1 && !(t.HLT_HIAK4PFJet80_v1 || t.HLT_HIAK4PFJet100_v1)) 
      {eec *= prescale;} 
    ...
    for (Long64_t ient = ev_first; ient < ev_last; ient++) {
        ...
        for (Int_t ijet = 0; ijet < t.nref; ijet++) {
            ...
            // if (skipMC(t.jtpt[ijet], t.refpt[ijet], t.pthat)) continue;
              ...
              if (skipMC_event(t.jtpt[ijet], t.pthat)) continue;
...
void create_files_for_template_fit(Int_t RunN = 3, Int_t dataType = 2, Float_t pT_low = 80, Float_t pT_high = 200,Float_t etaCut = 1.9,Int_t n = 1, bool btag = true, bool isMC = true, Double_t btagWP = -1){
 //gSystem->Load("libGenVector.so"); // load at prompt: gSystem->Load("libGenVector");
...
// --  new make_templates() with cfg
  make_templates(cfg,  0, 10); 

  // -- Make templates 
  // make_templates(RunN, filename, output_folder, output_hist, domain, pT_low, pT_high, etaCut, n, btag, btagWP, isMC, dataType, 0, 1e+04);
