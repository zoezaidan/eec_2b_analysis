...
// to load the function: make_templates() + other headers  
#include "create_files_for_template_fit_copy_runCodor.cpp" 

void run_condor_job(Int_t job_idx,
                   Int_t n_jobs, 
                  Int_t dataType,
                  Float_t pT_low,
                  Float_t pT_high,
                  ...
                  Int_t btag_int,
                  Int_t isMC_int,
                  Int_t applyHLT80_int,
                  Int_t applyHLT40_int) 
{

  bool btag = (btag_int != 0);
  bool isMC = (isMC_int != 0);
  bool applyHLT80 = ( applyHLT80_int != 0);
  bool applyHLT40 = ( applyHLT40_int != 0);



  TString filename, output_hist;
  TString output_folder = "/home/llr/cms/shatat/CMSAnalysis/eec_2b_analysis/Test_vs_Zoe_code/Zoe_version/eec_2b_analysis-main/condor/condor_output/"; 
  TString domain = ".root";
  ...
  // -- compute entries range 
  Long64_t chunk   = (n_total + n_jobs - 1) / n_jobs; //  ~ number of entries per job // why this way of defintion ? 
  ...
  make_templates(filename, output_folder, output_hist, domain,
                 ...
                 ev_first, ev_last, job_idx, applyHLT80, applyHLT40);
}
