...
for primary_dataset in $(seq 0 4); do
  input_dir=/data_CMS/cms/mnguyen/bJetAggRun3/PPRef2024/HardProbes/${primary_dataset}/UParTV2_chunks

  for i in $(seq 0 9); do
    block=$(printf "000%d" "${i}")
    script="${SCRIPT_DIR}/jobs_${job_index}.sh"
    input="${input_dir}/merged_block_${block}_UParTV2_rho.root"
...
if [ -d "\${OUTDIR}" ] && find "\${OUTDIR}" -maxdepth 1 -type f -name '*.root' -print -quit | grep -q .; then
  echo "ERROR: ROOT output already exists; refusing to overwrite: \${OUTDIR}"
  exit 3
fi
...
root -l -b -q -e "gSystem->Load(\"\${WORK}/create_files_for_template_fit_cpp.so\"); create_files_for_template_fit(3,0,80,9999,2,1,true,false,0.712,true,false,true,0,-1,\"\${INPUT}\",\"\${OUTDIR}\")"
EOF
