...
#include "tTree_Run3.h"
#include "binning_histos_small.h"
#include "Help_Functions.h"
#include "Draw_EEC.h"


void do_template_fit_combined(const TString &HighEGdata_name, const TString &LowEGdata_name, TString &templates, TString &templates_bjet, TString pT_selection, TString folder, TString &fout_name, bool& alsoLowEG, Variation ivar){
    /*
...
void template_fit_Run3(){

    //Get data and mc labels
    TString pT_selection = "80_200";// full range 
    TString folder = "/home/llr/cms/shatat/CMSAnalysis/eec_2b_analysis/TemplateFit_Run3/";

    // Add LowEG data 
    bool alsoLowEG = false; // no data 
    ...
    TFile *foutputPlots_dijet = new TFile(Form("%s/%s", sDirname.Data(), sfoutputPlots_dijet.Data()), "RECREATE"); // // RECREATE For canvas output
    ...
    TString dataset_HG = ""; 
    TString dataset_LG = ""; 
        TString dataname = "none";
        TString templates_dijet = "/home/llr/cms/shatat/CMSAnalysis/eec_2b_analysis/condor_Run3/MergedJobResult/MergedAllFiles_Run3_WP90_template_for_fit_histos_3D_qcd_btag.root";
        TString templates_bjet = "";
        TString fout_name = "Run3MC_Templates_histos_3d_" + pT_selection +  ".root";

        // -- Draw Prefit templates for Run3 MC (qcd sample)
        cout << "Draw Run 3 QC Dsample Templates in slices (pt, dr) and produce nice plots" << endl;
        Draw_template_Run3(templates_dijet, pT_selection, folder, fout_name);
        cout << "Done drawing template plots for Run3 QCD MC" << endl;
...
/*
        ...
        for (int ivar = 0; ivar < 1; ivar++) // nominal only
        {
            TString newfout_name = varNames[ivar]+ "_" + fout_name;
            // -- Template fit 
            do_template_fit_combined (dataset_HG,dataset_LG,templates_dijet, templates_bjet,  pT_selection, folder, newfout_name, alsoLowEG,(Variation) ivar);
            
          
            // -- Loop to Draw Template fit result of S/B fraction plots 
        ...
        TFile *fsys = new TFile( Form("%s/Result_syst_uncert_templatefit.root", sDirname.Data()),"recreate");
            // Reopen file if not yet
            // foutputPlots_dijet->ReOpen("READ"); 
            // TFile *foutputPlots_dijet = new TFile(Form("%s/%s", sDirname.Data(), sfoutputPlots_dijet.Data()), "Read"); 
